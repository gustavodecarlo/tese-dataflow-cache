...
def get_metadata_table_representation(spark_session: SparkSession, dataflow: str, task: str, datasource: str, metadata: dict) -> DataFrame:
    sql = f'''
        SELECT *
        FROM {dataflow}
        WHERE {metadata.get("operation").get("filter").get("value_dnf")}
    '''
